...
void loop()
{
    ...
    bool show_demo = true;
    {
        ...
        ImGui::Text("Websocket Angle = %f", camera_angle);

        ImGui::Text("Application average %.3f ms/frame (%.1f FPS)", 1000.0f / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate);
        ImGui::End();

        if (first_loop)
            ImGui::SetWindowPos("Demo", ImVec2(0, 0));
    }
    ...
    if (show_demo_window)
    {
        ...
        if (first_loop)
            ImGui::SetWindowPos("Dear ImGui Demo", ImVec2(400, 0));
    }

    bool show_implot_demo = true;
    ImPlot::ShowDemoWindow(&show_implot_demo);
    if (first_loop)
        ImGui::SetWindowPos("ImPlot Demo", ImVec2(1000, 0));
